...
func (p UserRepo) HandleGETAllUser() (*[]User, error) {
	...
	result, err := p.db.Query("SELECT username,alamat,telp FROM user WHERE status=?", "A")
	if err != nil {
		log.Println(err)
		return nil, err
	}
	...
	for result.Next() {
		err := result.Scan(&d.Username, &d.Alamat, &d.Telp)
		if err != nil {
			log.Println(err)
			return nil, err
		}
...
func (p UserRepo) HandleUserLogin(username, status string) (*User, error) {
	results := p.db.QueryRow("SELECT username,password FROM user WHERE username=? AND status=?", username, status)

	var d User
	err := results.Scan(&d.Username, &d.Password)
	if err != nil {
...
func (p UserRepo) HandlePOSTUser(d User) (*User, error) {
	...
	stmnt, _ := tx.Prepare(`INSERT INTO user(username,password,alamat,telp) VALUES (?,?,?,?)`)
	defer stmnt.Close()

	_, err = stmnt.Exec(d.Username, d.Password, d.Alamat, d.Telp)
	if err != nil {
		log.Println(err)
		tx.Rollback()
		return nil, err
